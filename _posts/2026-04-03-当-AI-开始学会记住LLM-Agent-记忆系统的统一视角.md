...
事情没那么简单。研究者们发现了一个有趣的现象叫"lost in the middle"：当你把关键信息埋在长文本的中间位置时，模型往往会"视而不见"。注意力机制理论上能看到所有 token，但实际上它对头尾更敏感，中间的信息容易被稀释。更实际的问题是成本——注意力计算的复杂度是 ![O(n^2)](/assets/img/marsggbo/2026-04-03-当-AI-开始学会记住LLM-Agent-记忆系统的统一视角/a5e2df1c.jpg)，上下文翻倍意味着计算量翻四倍，这在生产环境中是不可接受的。
...
如果你用过大模型推理框架，对 KV Cache 一定不陌生。它的原理很简单：Transformer 在自回归生成时，每个新 token 都需要和之前所有 token 做注意力计算。如果每次都从头算，复杂度是 ![O(n^2)](/assets/img/marsggbo/2026-04-03-当-AI-开始学会记住LLM-Agent-记忆系统的统一视角/a5e2df1c.jpg)；但如果把之前 token 的 Key 和 Value 向量缓存起来，新 token 只需要算自己的部分，复杂度就降到了 ![O(n)](/assets/img/marsggbo/2026-04-03-当-AI-开始学会记住LLM-Agent-记忆系统的统一视角/b1e1fe9d.jpg)。
