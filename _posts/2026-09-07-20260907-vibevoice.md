...
图中最重要的不是两个 tokenizer，而是**同一条横向序列的顺序**：先放音频，再放它对应的文字；文字生成后不被丢弃，而是继续留在模型上下文里。因此下一轮既能看到新的声音，也能看到“之前谁说过什么”。
...
这也解释了为什么不能只盯着最右侧粗体：Google 的 `D0` 和 `Dfin` 文本相同，标签却可能在整段录音期间被重排。VibeVoice 的优势是直接给出一个较早的固定标签，代价则是它接受了流式约束，后面不会依靠完整录音再回头重算。
