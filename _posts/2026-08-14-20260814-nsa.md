...
这三个数字为什么 decode 反而是最大的？简单解释一下：

- **Forward（prefill）**：把整个 prompt 并行处理，compute-bound——FLOPs 少了，就快了，9×
- **Decode**：每次只生成一个新 token，需要从 HBM 里把所有历史 token 的 K/V 都读出来，**memory-bandwidth bound**。Sparse attention 跳过大量 KV 的 load，直接减少 HBM 读取量，对 bandwidth bound 的操作收益最大，所以反而是 11.6×
- **Backward**：梯度计算量最大，除了 attention 本身还有大量其他操作，sparse 的比例相对没那么显著，6×
