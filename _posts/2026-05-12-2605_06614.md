---
layout: post
title: "SkillOS：用RL训练一个会「管理技能库」的AI，让Agent真正越用越强"
date: 2026-05-12
tags: [LLM, Agent, RL, 论文解读]
---

# SkillOS：用RL训练一个会「管理技能库」的AI，让Agent真正越用越强

> 原文：[SkillOS: Learning Skill Curation for Self-Evolving Agents](https://arxiv.org/abs/2605.06614)
...
![SkillOS系统总览：Executor与Curator解耦设计](/assets/img/posts/2605_06614/page_002.png)
...
用 GRPO（Grouped Reward Policy Optimization）来优化，base model 是 Qwen3-8B，16 张 H100 训练约 3-5 天。
...
![ALFWorld主实验结果](/assets/img/posts/2605_06614/page_007.png)

几个关键结论：

**1. SkillOS 全面碾压 baseline**

- Qwen3-8B 作为 executor：SkillOS 平均成功率 **61.2%**，比最强 baseline ReasoningBank 的 55.7% 高 **+5.5 个点**，比无记忆 baseline 的 47.9% 高 **+13.3 个点**；
- Gemini-2.5-Pro 作为 executor：从 66.4% 提升到 **80.2%**，同样大幅领先；

**2. 8B 训练过的 curator 打赢 Gemini-2.5-Pro 直接做 curator**

这是最有意思的结论之一。SkillOS-gemini 用 Gemini-2.5-Pro 直接做 skill curator（不经 RL 训练），在 Qwen3-8B executor 下只有 50.7%；而我们 RL 训练过的 Qwen3-8B curator 达到 61.2%。

**单纯模型强不等于 skill curation 做得好**——frontier model 生成的技能可能和 executor 的使用习惯对不上，而 RL 训练让 curator 学会了"为 executor 量身定制"。

**3. 效率也提升了**

interaction steps 从 21.1 降到 18.9（Qwen3-8B 执行器），技能帮 agent 走了捷径，不用重复摸索。

### 4.2 跨任务泛化

如下图，SkillOS 在跨 task domain 迁移上也表现不错：

![跨任务泛化结果](/assets/img/posts/2605_06614/page_009.png)
...
![Ablation与技能演化分析](/assets/img/posts/2605_06614/page_010.png)
...
训练开始时，curator 大量 `insert`（建库），随着训练推进，`update` 比例不断上升，`insert` 下降——模型从"疯狂存技能"变成了"精炼、迭代技能"。`delete` 始终很少但在增长，说明 compression reward 在慢慢起作用。
