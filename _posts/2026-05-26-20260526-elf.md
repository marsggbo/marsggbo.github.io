...
### 2.1 先定义符号：$\boldsymbol{x}$、$\boldsymbol{\epsilon}$、$\boldsymbol{z}_t$ 分别是什么？

在整篇文章里我们会反复用到三个核心变量，先把它们定义清楚：

| 符号 | 含义 | 图像生成里是什么 | 文本生成里是什么 |
|------|------|:---:|:---:|
| $\boldsymbol{x}$ | 干净数据 | 一张清晰图片的像素值（或 latent） | 一段话的 embedding 向量序列 |
| $\boldsymbol{\epsilon}$ | 纯随机噪声 | 和图片同形状的高斯随机数 | 和 embedding 同形状的高斯随机数 |
| $\boldsymbol{z}_t$ | $t$ 时刻的中间状态 | 介于"干净图片"和"纯噪声"之间的混合物 | 介于"干净 embedding"和"纯噪声"之间的混合物 |

**Diffusion 模型的目标**：学会从 $\boldsymbol{z}_t$（一团被噪声污染的数据）里恢复出 $\boldsymbol{x}$（干净数据）。一旦模型学会了这件事，生成时就可以从纯噪声 $\boldsymbol{\epsilon}$ 出发，一步步"去噪"到 $\boldsymbol{x}$，凭空创造出新数据。

### 2.2 DDPM：加噪去噪的基本思路

具体来说，DDPM（Denoising Diffusion Probabilistic Model）把"加噪"定义为：

$$\boldsymbol{z}_t = \sqrt{\bar\alpha_t}\,\boldsymbol{x} + \sqrt{1-\bar\alpha_t}\,\boldsymbol{\epsilon}$$

- $t$ 从 $0$ 到 $T$（比如 1000）
- $\bar\alpha_t$ 是一个随 $t$ 递减的系数。$t=0$ 时 $\bar\alpha_0 \approx 1$，所以 $\boldsymbol{z}_0 \approx \boldsymbol{x}$（几乎全是干净数据）；$t=T$ 时 $\bar\alpha_T \approx 0$，所以 $\boldsymbol{z}_T \approx \boldsymbol{\epsilon}$（几乎全是噪声）

一个具体例子：**图像生成**。假设 $\boldsymbol{x}$ 是一张猫的图（$256\times256\times3$ 的浮点数组），$\boldsymbol{\epsilon}$ 是同形状的随机数。$t=500$ 时 $\boldsymbol{z}_{500}$ 大约是"能隐约看到有猫轮廓的噪声图"，到 $t=1000$ 就完全看不出任何图案了。

训练时，网络看到 $(\boldsymbol{z}_t, t)$，学习预测加入的噪声 $\boldsymbol{\epsilon}$：

$$\mathcal{L}_\text{DDPM} = \mathbb{E}_{t, \boldsymbol{x}, \boldsymbol{\epsilon}} \left[ \| \boldsymbol{\epsilon}_\theta(\boldsymbol{z}_t, t) - \boldsymbol{\epsilon} \|^2 \right]$$

这叫 **ε-prediction**。既然知道了 $\boldsymbol{\epsilon}$，结合公式 $\boldsymbol{z}_t = \sqrt{\bar\alpha_t}\,\boldsymbol{x} + \sqrt{1-\bar\alpha_t}\,\boldsymbol{\epsilon}$ 就能算出 $\boldsymbol{x}$。

**DDPM 的缺点**：加噪路径不是直线（中间要经过一条弯曲的轨迹），所以去噪必须走很多小步（200~1000 步），推理非常慢。

### 2.3 Flow Matching：走直线，少走路

Flow Matching（FLUX、SD3 背后的技术）改用**直线路径**：

$$\boldsymbol{z}_t = t \cdot \boldsymbol{x} + (1-t) \cdot \boldsymbol{\epsilon}, \quad t \in [0, 1]$$

这就是一个最简单的线性插值。把它理解成"一辆从 $\boldsymbol{\epsilon}$ 出发、匀速直线开向 $\boldsymbol{x}$ 的车"：$t=0$ 时在起点（纯噪声），$t=1$ 时到终点（干净数据）。

这辆车的速度是多少？对 $t$ 求导：

$$\boldsymbol{v} = \frac{d\boldsymbol{z}_t}{dt} = \boldsymbol{x} - \boldsymbol{\epsilon}$$

速度 $\boldsymbol{v}$ 是常数（匀速直线运动嘛），等于"终点减起点"。

训练时，网络学习这个速度场 $\boldsymbol{v}_\theta(\boldsymbol{z}_t, t) \approx \boldsymbol{x} - \boldsymbol{\epsilon}$。推理时就简单了——从 $\boldsymbol{z}_0 = \boldsymbol{\epsilon}$（纯噪声）出发，按预测的速度一步步往前走（解 ODE）：

$$\boldsymbol{z}_{t+\Delta t} = \boldsymbol{z}_t + \Delta t \cdot \boldsymbol{v}_\theta(\boldsymbol{z}_t, t)$$

因为真实路径是直线，**32 步 Euler 积分就能走得很准**——比 DDPM 的 1000 步快 30 倍。这也是 ELF 高效的根基。

### 2.4 网络预测什么？三种等价的选择

前面说了 $\boldsymbol{z}_t = t\boldsymbol{x} + (1-t)\boldsymbol{\epsilon}$，$\boldsymbol{v} = \boldsymbol{x} - \boldsymbol{\epsilon}$。这两个方程有三个未知量（$\boldsymbol{x}$、$\boldsymbol{v}$、$\boldsymbol{\epsilon}$），知道一个就能算出另外两个：

| 预测目标 | 网络输出的是 | 怎么推算 $\boldsymbol{x}$ | 直觉含义 |
|:---:|:---:|:---:|:---:|
| **x-prediction** | $\hat{\boldsymbol{x}}$ | 已经是 $\boldsymbol{x}$ | "直接预测干净数据长什么样" |
| **v-prediction** | $\hat{\boldsymbol{v}}$ | $\hat{\boldsymbol{x}} = \boldsymbol{z}_t + (1-t)\hat{\boldsymbol{v}}$ | "预测从噪声到数据的运动速度" |
| **ε-prediction** | $\hat{\boldsymbol{\epsilon}}$ | $\hat{\boldsymbol{x}} = \frac{\boldsymbol{z}_t - (1-t)\hat{\boldsymbol{\epsilon}}}{t}$ | "预测混入了多少噪声" |

在图像生成里，三种方式效果差不多。但 **ELF 只能用 x-prediction**——原因在第 4 节详细讲。
