...
1. edges: `1/0`表示对应节点之间是否相连，所以有 $2^{21}$种连接方式
2. ops: 除去`in & out` 两个节点，每个节点都有3个候选操作,所以总共有 $3^5$中组合

所以总共搜索空间有 $2^{21}\times 3^5 \approx 510M$ 中可能的cell结构。不过因为规定了edge总数不能大于9，另外减去重复的，最后总共有将近 **423K** 个不同的结构图。
