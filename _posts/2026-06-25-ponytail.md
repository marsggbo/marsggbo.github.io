---
layout: post
title: "AI agent 写代码动不动就 100 行？这个 5w 星的 ponytail 用一个 hook 把它治服了"
date: 2026-06-25
tags: [LLM, AI Agent, 开源项目, 源码解析]
---

# AI agent 写代码动不动就 100 行？这个 5w 星的 ponytail 用一个 hook 把它治服了
...
你有没有让 Claude Code 或者 Cursor 写过一个看似简单的功能？

比如让它"写一个日期选择器"。然后你回过头来一看，它给你装了 flatpickr，写了个 wrapper 组件，配了一份样式表，还顺便和你讨论起了时区问题——五十行代码起步。其实浏览器自带一个 `<input type="date">` 就完事了。

这不是个别现象。**作为一个研究 LLM 推理优化的牛马**，我自己每天和 AI agent 打交道，最深的感受就是这帮 agent 写代码有一个"过度工程"的本能：能写一个 class 绝不写一个函数，能写五十行绝不写一行，能引入一个新依赖绝不去翻翻 stdlib。说白了，它们的训练语料里塞满了 over-engineered 的"最佳实践"，所以默认行为就是把简单问题搞复杂。

最近 GitHub 上有个名字很骚的项目 `ponytail`（马尾辫）火得离谱——一个月之内冲到 **55.1k stars**，挂在 Trendshift 周榜和日榜上下不来。它的定位也很有意思，不是又一个 agent 框架，也不是又一个 IDE 插件，而是给你的 AI agent 强行塞一个"扎马尾辫、戴椭圆眼镜、比 Git 还老的资深工程师人格"。
...
今天想和大家聊聊这个项目：它为什么火、效果到底有多猛、以及——重点——**它的实现到底有什么巧妙之处**。读完你会发现，ponytail 真正有意思的不是它那段"一行胜过五十行"的口号，而是它通过一组 hook 把这套哲学**持久化注入**到 agent 的每一次会话里的工程实现。
...
![ponytail vs baseline agentic 评测](/assets/img/posts/ponytail/benchmark-agentic.svg)
...
![single-shot 跨模型对照](/assets/img/posts/ponytail/benchmark-3model.svg)
