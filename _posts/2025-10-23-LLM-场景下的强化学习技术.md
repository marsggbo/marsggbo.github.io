...
### GRPO 损失函数（简化版）$$\mathcal{L}_{\text{GRPO} = -\mathbb{E}_x \left[ \log \frac{\sum_{y \in \text{good} \exp(\beta \log \frac{\pi_\theta(y|x)}{\pi_{\text{ref}(y|x)})}{\sum_{y \in \text{all} \exp(\beta \log \frac{\pi_\theta(y|x)}{\pi_{\text{ref}(y|x)})} \right]$$
