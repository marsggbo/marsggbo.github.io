...
上图表示的是一个cell的结构。一个cell由若干个节点（node）组成，每组节点之间通过若干条边（edge）连接起来，每条edge表示不同的操作（用![o](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/efe231e3.jpg)表示），比如卷积或者池化操作等。DARTS的想法是每条edge都有一个权重（用![\alpha](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/b5edc039.jpg)表示），而且权重是可以通过梯度更新的，最后会根据权重来选择节点之间的操作，计算公式如下：

![\bar{o}^{(i, j)}(x)=\sum_{o \in \mathcal{O}} \frac{\exp \left(\alpha_{o}^{(i, j)}\right)}{\sum_{o^{\prime} \in \mathcal{O}} \exp \left(\alpha_{o^{\prime}}^{(i, j)}\right)} o(x) \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/ad9383e9.jpg)
...
![\begin{array}{cl} y &= z_1 + z_2+z_3 \\ &=w_1o_1(x)+w_2o_2(x)+w_3o_3(x) \\ \Rightarrow & \frac{\partial y}{\partial w_1}=o_1(x),\frac{\partial y}{\partial w_2}=o_2(x),\frac{\partial y}{\partial w_3}=o_3(x) \end{array} \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/6f076be9.jpg)

* 以一定概率随机采样的表达式（右边表示概率），可以看到这种随机采样无法求出梯度。

![y=\left\{\begin{array}{l} o_{1}(x),  \,\,\,(p=w_1) \\ o_{2}(x),  \,\,\,(p=w_2)  \\ o_{3}(x),  \,\,\,(p=w_2)  \end{array}\right. \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/6e1e80b2.jpg)
...
然后再假设我们可以根据某种分布对每个权重采样一个随机值，比如三个权重对应的采样的随机值分别是![\epsilon=[0.5,0.6,0.05]](https://www.zhihu.com/equation?tex=%5Cepsilon%3D%5B0.5%2C0.6%2C0.05%5D),我们把这些随机值和权重相加之后得到![\hat{W}=[0.1+0.5,0.2+0.6,0.7+0.05]=[0.6,0.8,0.75]](https://www.zhihu.com/equation?tex=%5Chat%7BW%7D%3D%5B0.1%2B0.5%2C0.2%2B0.6%2C0.7%2B0.05%5D%3D%5B0.6%2C0.8%2C0.75%5D)。所以![\hat{W}=W+\epsilon, \epsilon \thicksim P(某种分布)](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/0c6b8a21.jpg)，一般这个分布可以是0到1之间的均匀分布，即![\epsilon \thicksim U(0,1)](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/ecd33fd2.jpg)。

之后我们对采样随机值后的权重分布取![argmax(\hat{W})](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/448260ad.jpg)的话应该是选择第二个操作，当然这种概率是比较小的，这个也叫Gumbel-Max trick。可是argmax也有无法求导的问题，因此可以使用softmax来代替，也就是Gumbel-Softmax trick，那么有如下计算公式（![\tau](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/8787bfea.jpg)表示温度系数，类似于知识蒸馏里的温度系数，也是用来控制分布的平滑度）

![\begin{array} \hat{w}_1&=\frac{e^{\hat{w}_1/\tau}}{\sum_{i=1}^3e^{\hat{w}_i/\tau}} \\ &=\frac{e^{({w}_1+\epsilon_1)/\tau}}{\sum_{i=1}^3e^{({w}_i+\epsilon_i)/\tau}} \\ s.t. & \epsilon \thicksim U(0,1) \end{array} \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/b40cb305.jpg)

我们现在再来看看使用gumbel softmax后的求导表达式：

![\begin{array} .y &= \hat{w}_1o_1(x)+\hat{w}_2o_2(x)+\hat{w}_3o_3(x)\\ &\Rightarrow \frac{\partial y}{\partial w_1}=\frac{\partial y}{\partial \hat{w}_1}\frac{\partial \hat{w}_1}{\partial w_1}, ... \end{array} \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/c933548f.jpg)
...
* 对于网络输出的一个n维向量![v](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/2bd40b4a.jpg),生成n个服从均匀分布![U(0,1)](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/582ebd37.jpg)的独立样本![\epsilon_1,...,\epsilon_n](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/c74b9ef1.jpg)
* 通过![G_i=−log(−log(\epsilon_i))](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/2707368f.jpg)计算得到![G_i](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/2700ef85.jpg)
...
![\sigma_{\tau}\left(v_{i}^{\prime}\right)=\frac{e^{v_{i}^{\prime} / \tau}}{\sum_{j=1}^{n} e^{v_{j}^{\prime} / \tau}} \\](/assets/img/marsggbo/2020-07-03-Gumbel-softmax在可微NAS的作用是什么/55990480.jpg)
