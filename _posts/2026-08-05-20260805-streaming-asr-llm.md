...
具体做法（对应图中的 Chunking + Speech encoder + Adaptor 三层）：每次送进 Conformer 的是「最近 1.6 秒历史 + 当前 0.4 秒」拼在一起，总共 2 秒。Conformer 处理完输出所有帧，**只保留对应"当前 0.4 秒"的那部分，历史帧的输出直接丢掉**。每个 chunk 不管是第几个，最终都产出固定帧数（0.4 秒对应多少帧就是多少帧），多个 chunk 的输出按顺序拼接，得到持续增长的 audio embedding 序列：

```
chunk 1 (0~0.4s):   输入 [padding, 0~0.4s]    → Conformer → 保留后 10 帧
chunk 2 (0.4~0.8s): 输入 [0~0.4s, 0.4~0.8s]  → Conformer → 保留后 10 帧（丢掉前 10 帧）
chunk 3 (0.8~1.2s): 输入 [0~0.8s, 0.8~1.2s]  → Conformer → 保留后 10 帧（丢掉前 20 帧）
...（历史超过 1.6s 只保留最近 1.6s）
```

历史 context 的作用是让 Conformer 在对当前帧做 attention 时能"看到前面说了什么"，从而编码质量更好。不保留它的输出是因为之前的 chunk 已经输出过这些帧，重复输出会打乱序列。

**为什么 1.6 秒历史 context 不增加延迟？** 关键在于：延迟 = 你要等多久才能开始处理。1.6 秒历史是已经发生的音频，处理第 k 个 chunk 的时候它早就在缓冲区里了，不需要等任何东西。反过来，如果要看"未来 0.4 秒"来辅助当前 chunk，就必须等未来那段音频说完——每看一个 chunk 的未来就多等 0.4 秒。所以整个系统的最小延迟只由 **chunk 大小（0.4 秒）** 决定，跟 context 窗口多大完全无关。

所以"0.4 秒切成 chunk"的意思是：**音频来了 0.4 秒就处理一批，不是等整段说完**。第一个字要等的时间取决于 MoChA 何时触发，理论下界就是一个 chunk 的时长（0.4 秒），实际上几个 chunk 之后就能开始出字。

关于未来上下文缺失的问题：1.6 秒的历史窗口只能向后看，不能向前看。确实存在"需要后面的音频才能确定这个字"的情况（比如多音字、上下文依赖强的词），这种情况下流式就会出错，这也是精度损失的主要来源。没有银弹，这是流式 ASR 的基本代价。
...
> ![动手学AutoML书籍封面](https://github.com/marsggbo/marsggbo.github.io/blob/master//assets/img/posts/20260805-streaming-asr-llm/img/book_cover_automl.png?raw=true)
