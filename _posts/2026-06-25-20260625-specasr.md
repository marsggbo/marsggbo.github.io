...
如下图，LibriSpeech test-clean 上的延迟分解数据也印证了这一点：

![解码延迟占比分析](/assets/img/posts/20260625-specasr/010_Figure_7.png)

LLM decoder 的 decoding 延迟占绝对大头，这也是为什么 speculative decoding 是有效的切入点。
...
实验结果非常直观（如下图 5b，小模型在 ASR 任务里的 acceptance rate 明显高于纯文本任务）：

![ASR vs 文本任务的 speculative acceptance 对比](/assets/img/posts/20260625-specasr/001_a.png)
...
主要速度对比结果（相比 autoregressive baseline）：

- **test-clean**：3.79×（Vicuna-13B as target）
- **test-other（噪声数据）**：3.04×-3.07×
