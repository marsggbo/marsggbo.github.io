...
关键在于，参数 buffer 里存的是**指针而不是数据本身**——权重、KV cache、中间激活这些大块头都不在图里，图只是"指向"它们的地址。而且 vLLM 里不同 batch size 的图共享同一套预分配的输入输出 buffer（反正同一时刻只有一张图在 replay），所以几百张图的额外开销就是几百份元数据。
