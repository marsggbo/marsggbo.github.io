...
迭代遍历通常会用到 deque (double-ended queue 双端队列)，为了实现层序遍历，那逻辑很简单，就是先把第 i 层（从 0 开始）的所有节点插入到队列里，并记下数量 ![n_i](/assets/img/marsggbo/2025-10-21-二叉树的前中后序遍历到底是什么/b58f1a5a.jpg)，然后遍历这些数量的节点。 遍历第 i 层节点的同时继续把这些节点的左右节点插入到队列了，遍历完了就是就等价于把第![i+1](/assets/img/marsggbo/2025-10-21-二叉树的前中后序遍历到底是什么/d384689a.jpg)层节点也都插入到队列里了，类似地也记录下这一层的节点数量，重复迭代就实现了层序遍历。
