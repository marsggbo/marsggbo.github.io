...
>
> 用线程局部变量传递的好处：vllm 替换了 attention 层的实现（调用 FlashAttention kernel 的版本），这些 attention 层知道去全局上下文里取 `attn_metadata`；但模型的其他部分（embedding、FFN、layernorm）完全不知道有这个东西，接口和 HuggingFace 保持一致。这是**依赖注入**的一种变体——把依赖藏在上下文里而不是参数里。
>
