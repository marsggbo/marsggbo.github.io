...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → output_processor.process_outputs(engine_core_outputs)  ← 第七步在这里
#   → engine_core.step() → [在 step() 之前] input_processor.process_inputs()
...
```python
# LLM.generate() → _run_engine() → LLMEngine.add_request()
#   → engine_core.add_request() → scheduler.add_request()
...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → engine_core.step() → scheduler.schedule()
...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → engine_core.step() → executor.execute_model()
# EngineCore.step()  —  vllm/v1/engine/core.py
...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → engine_core.step() → executor.sample_tokens() → Sampler.forward()
# EngineCore.step()（精简）
...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → engine_core.step() → scheduler.update_from_output()
# EngineCore.step()（精简）
...
```python
# LLM.generate() → _run_engine() → LLMEngine.step()
#   → output_processor.process_outputs() → IncrementalDetokenizer.update()
# LLMEngine.step()（精简）
