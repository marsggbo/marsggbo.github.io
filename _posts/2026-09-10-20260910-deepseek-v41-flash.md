...
这句话放在一份以 KV Cache Compression 为标题的报告里，多少有点反直觉。前半部分明明塞进了 CED、CSA2、FP4 KV、Bounded Replay 等一整套架构和系统改动；到了后训练，作者却刻意把算法上的新意收了起来，把注意力放到数据、任务和环境是如何被造出来的。

模型一旦开始调用工具、处理代码仓库和网页、跑很多轮任务，先得付得起上下文不断增长的成本：
...
报告所说的“没有新算法”只针对后训练。CED、CSA2、FP4 KV 这些仍是实打实的架构与系统改动；但进入 SFT → RL → OPD 后，整体配方沿用了已有做法，变化发生在模型面对的材料上。
...
在这种定义下，数据管线包含的不是更多 prompt，而是更多可验证的任务、更多不同的交互环境，以及一套能持续过滤、去重和调节难度的机制。报告在第 25 页的判断也因此比“数据很重要”更强：在一个固定且并不特殊的优化流程下，合成数据和环境的**规模、多样性与可验证性**几乎解释了观察到的全部能力增益。这当然仍是作者对自身训练系统的归因，不是普适的因果定律；但它把 Agent 后训练的重心，从设计一个更花哨的 reward，推到了如何长期稳定地生产好任务上。
