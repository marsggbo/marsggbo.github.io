---
layout: post
title: "DeepSeek V4.1 Flash | 后训练没有新算法，Agent 能力从哪来？"
date: 2026-09-10
...
但报告里还有另一条很容易被 “KV Cache Compression” 标题盖住的线索：DeepSeek 明说，**后训练没有引入新的算法，主要变化落在数据管线**。这不是否认 CED、CSA2、FP4 等架构改动；更准确地说，报告把 V4.1 的工作拆成两条问题线：

| 问题 | 报告给出的主要答案 |
| --- | --- |
| 长程 Agent 为什么能更便宜地跑？ | CED、CSA2、FP4 与 Bounded Replay，把 prefill、KV 存储和缓存恢复分开优化 |
| 后训练没有新算法，Agent 能力为什么还会涨？ | 把训练对象从单轮问答变成可验证任务、可运行环境与可持续回审的 rollout 数据 |

后者才是本文标题的提问。后文讲 KV Cache，不是在回避这个问题；恰恰相反，长上下文与低缓存成本让更长、更真实的 Agent rollout 变得可承受，而数据和环境管线决定这些 rollout 能否真正变成有效训练信号。
...
## 5. 没有新算法，Agent 能力为什么还能涨？

### 5.1 变量没有换到优化器，而是换到了训练样本与环境
...
作者实际扩展的不是 reward 公式，而是下面这条训练数据闭环：

```text
真实工作流、失败反馈、复杂代码仓库
            ↓
构造任务：问题 + 可运行环境 + 参考解/验证器
            ↓
Agent 在工具与容器中 rollout，产生可检查的完整轨迹
            ↓
按正确性、难度、去重与可被钻空子的风险做质检
            ↓
失败轨迹与新问题回流，生成下一轮更有区分度的任务
```

这条闭环解释了“数据管线”不是简单多抓一点语料。它把任务、环境、验证与失败样本绑在一起，让 RL 的 reward 能对应一个可复现的外部结果。作者实际扩展的是三件事：
