...
上一篇讲了 `EngineCore.step()` 的调用链：调度 → GPU forward → 采样 → 状态更新。这篇要拆开"调度"这一步的内核——KV Cache 怎么管、Chunked Prefill 怎么工作，以及在线 serving 场景下整个架构为什么要用独立进程。

先把这几块代码在哪里、怎么串起来交代清楚：

```python
# EngineCore.__init__()  —  vllm/v1/engine/core.py:99
def __init__(self, vllm_config):
    self._initialize_kv_caches(vllm_config)   # 算显存、分配 GPU tensor
    self.kv_cache_manager = KVCacheManager(   # 块管理器：负责分配/释放/前缀缓存
        vllm_config, num_gpu_blocks, ...)     # vllm/v1/core/kv_cache_manager.py:110
    self.scheduler = Scheduler(
        vllm_config, self.kv_cache_manager)   # 调度器持有 KVCacheManager
...
def schedule(self):
    for request in self.running:
        ...
        if not ok:
            self._preempt(request)   # 显存不够，踢回等待队列
...
def _prepare_inputs(self, scheduler_output):
    # 把逻辑块编号映射成 GPU tensor 里的物理槽位
    slot_mapping = compute_slot_mapping(block_table, seq_lens, ...)  # [N_tokens]
...
`KVCacheManager` 夹在调度器和 GPU 执行之间：调度器决定分配哪些块，GPU 执行时通过 `block_table` 和 `slot_mapping` 找到物理位置。这几块是相互咬合的，下面逐个拆开。
...
```python
# vllm/v1/attention/backends/flash_attn.py  FlashAttentionImpl.forward()（精简）
...
![AsyncLLM + ZMQ 前后端通信时序：从 HTTP 请求到逐 token 流式输出](/assets/img/posts/20260706-vllm-v1-03-kvcache/diagram_async_zmq.png)
