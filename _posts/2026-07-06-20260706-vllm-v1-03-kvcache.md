...
每个块对应 GPU KV Cache tensor 的一个切片：

```python
# kv_cache 的 shape: [num_blocks, 2, block_size, num_kv_heads, head_dim]
kv_cache[block_id, 0]  # K cache，shape [block_size, num_kv_heads, head_dim]
kv_cache[block_id, 1]  # V cache，shape [block_size, num_kv_heads, head_dim]

# 具体例子：假设 block_size=16, num_kv_heads=8, head_dim=128, 共分配了 1000 个块
# kv_cache.shape = [1000, 2, 16, 8, 128]
# kv_cache[42, 0] 就是第 42 号物理块的 K cache，存了 16 个 token 的 K 向量
# kv_cache[42, 1] 就是第 42 号物理块的 V cache，存了 16 个 token 的 V 向量
```
...
具体例子：`max_num_batched_tokens=256`，当前有 2 个 decode 请求（各消耗 1 token），remaining_token_budget = 256 - 2 = 254。新来一个 1000-token prompt：
...
一个 1000-token 的 prompt 被自动切成 4 步，每步和正在 decode 的请求混在同一个 batch 里执行，Req B 的 decode 全程没有被阻塞：
