...
对 GPU 来说，这就是一次 forward pass，处理一个 `[255]` 长的 1D tensor。FlashAttention 的 varlen 接口会按 `cu_seqlens` 的边界分别处理每个请求的 attention，prefill 段和 decode 段互不干扰。Req B 始终在每步的 batch 里，每步都能产出一个新 token，不会因为 Req A 的 prefill 被阻塞。
