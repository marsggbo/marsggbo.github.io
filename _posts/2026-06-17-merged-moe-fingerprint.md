---
layout: post
title: "AAAI'26 | RouteMark：给融合 MoE 里的每个专家，盖一枚抹不掉的「路由指纹」"
date: 2026-06-17
tags: [MoE, Model Merging, 模型版权, IP, 论文解读]
---

# AAAI'26 | RouteMark：给融合 MoE 里的每个专家，盖一枚抹不掉的「路由指纹」

> 原文：[RouteMark: A Fingerprint for Intellectual Property Attribution in Routing-based Model Merging](https://arxiv.org/abs/2508.01784)（AAAI 2026）
...
今天就想借这个由头，聊聊我们最近被 AAAI 2026 接收的一个工作——**RouteMark**，它专门解决「融合之后怎么追溯版权」这件事。但在讲我们的方法之前，得先把几个概念递进着捋清楚。
...
因为每个 expert 的贡献是随着输入和层数动态变化的，单个 expert 的影响被「路由」这层机制给搅浑了。更麻烦的是，MoE 这种模块化结构，给了对手非常多「洗白」的手段。假设有人把你的模型当 expert 偷偷塞进了他的 merged MoE，他可以：

- **参数级篡改**：把你的 expert 继续 fine-tune 一下、做个 pruning 压缩一下、甚至把隐藏维度做个 permutation（置换）打乱参数对齐；
- **结构级篡改**：把你的 expert 替换掉一部分、删掉一部分、或者注入一堆冗余 expert 来混淆视听。

这些操作都能在**不影响整体性能**的前提下，把「这是谁的模型」这条线索抹得干干净净。
...
一句话总结这个困境：**我们需要一种能精确到「单个 expert 级别」、又能扛住各种篡改的指纹。** 这就是 RouteMark 想做的事。

---

## 5. 我们的 insight：会变的都靠不住，路由偏好不会变

讲方法之前，我想先把**核心观察**讲透，因为整个方法的合理性全都建立在这一个点上。

我们的出发点是一个很朴素的问题：**融合之后，一个 expert 身上还有什么东西是「抹不掉」的？**

权重可以剪、可以置换、可以微调；激活可以被搅浑。这些都靠不住。但我们发现了一个非常稳的东西——

> **task-aligned routing（任务对齐的路由行为）：融合之后，每个 expert 依然对它「原始训练领域」的输入最敏感，router 还是会大概率把对应任务的输入派给它。**

我们做了个实验来验证这件事。把五个分别在 SUN397、Cars、RESISC45、EuroSAT、SVHN 上 fine-tune 的 CLIP 专家融合成一个 MoE，然后看它的路由行为，结果如下图：
...
这个现象为什么重要？因为它给了我们一个**功能性的、而非参数性的**指纹来源。
...
几个我觉得最能说明问题的数字（victim 是 5 个专家的 CLIP MoE）：
...
- **结构篡改（替换/增加/删除 1~3 个专家）**：被复用的真专家平均相似度稳定在 **0.94 以上**，而新塞进来的外来专家相似度分布很均匀（margin 只有 0.05~0.10），**不会被错误归属**。
...
照例，自己的工作也得自己先挑挑刺，免得吹过头。我觉得 RouteMark 至少还有这么几个可以接着做的点：

1. **探针数据是有前提的。** 我们的指纹要在「expert 原始训练领域」的数据上构造，这等于默认你大致知道每个 expert 是干嘛的。如果对手把一个领域完全未知的模型塞进去，怎么构造探针、怎么追溯，是个开放问题。

2. **我们研究的融合范式是特定的一种。** 论文里的 MLP 融合用的是一个全局 router 输出共享的融合权重（所有 token 共用一套），跟标准的 token-level MoE 那种逐 token 路由还不完全一样。换到真正的 token-level MoE merging 上，路由行为会不会一样稳，值得再验证。

3. **目前只在 CLIP 视觉专家上验证。** LLM 的 MoE merging、多模态 MoE merging 还没试，这也是论文结尾明确写的未来方向。

4. **还没考虑「自适应攻击」。** 如果对手知道 RouteMark 的存在，故意去重训 router、把这种 task-aligned 的路由偏好人为抹平，指纹还守不守得住？这是攻防博弈里绕不开的一环，我们暂时还没深挖。

但我反而觉得，**正是因为这些坑还没填完，这个方向才有得做。** model merging 只会越来越普及，而「融合时代的版权追溯」这个命题，现在还几乎是一片空地。RouteMark 算是在这片空地上插了第一面旗——证明了「路由行为」这条路是走得通的。后面怎么把它做得更通用、更鲁棒，就看大家的了。
