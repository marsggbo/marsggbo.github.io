...
几个最能说明问题的数字（victim 是 5 个专家的 CLIP MoE）：
...
2. **研究的融合范式是特定的一种。** 论文里的 MLP 融合用的是一个全局 router 输出共享的融合权重（所有 token 共用一套），跟标准的 token-level MoE 那种逐 token 路由还不完全一样。换到真正的 token-level MoE merging 上，路由行为会不会一样稳，值得再验证。
