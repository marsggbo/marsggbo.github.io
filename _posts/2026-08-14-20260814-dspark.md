...
这里 $p_k^d$ 是 drafter 在第 $k$ 个位置的输出分布，$p_k^t$ 是目标模型在同位置的输出分布，$\|\cdot\|_1$ 是 L1 范数（逐元素绝对差求和）。

直觉是：投机解码的 rejection sampling 机制保证了 per-step acceptance rate 恰好等于 $1 - \frac{1}{2}\|p^d - p^t\|_1$——两个分布越接近，TV 距离越小，token 被接受的概率越高。用这个值作为 $c_k$ 的监督目标，模型就学会了"我的草稿和大模型有多像"，而不只是"我猜的 token 对不对"。两者的区别很关键：accuracy 高不等于 acceptance rate 高，因为即使猜对了 top-1，如果分布差异大，rejection sampling 也可能拒掉。
