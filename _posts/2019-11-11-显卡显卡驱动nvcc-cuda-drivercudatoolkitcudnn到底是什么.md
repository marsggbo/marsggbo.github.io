...
![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/589aa057.jpg)
...
* **CUDA Samples**: 演示如何使用各种CUDA和library API的代码示例。可在Linux和Mac上的`samples/`目录中获得，Windows上的路径是`C：\ProgramData\NVIDIA Corporation\CUDA Samples`中。在Linux和Mac上，`samples/`目录是只读的，如果要对它们进行修改，则必须将这些示例复制到另一个位置。
...
![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/f2efac14.jpg)
...
![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/994a29a9.jpg)
...
* **上下文管理** 上下文管理可以通过driver API完成，但是在runtime API中不公开。相反，runtime API自己决定为线程使用哪个上下文:
...
![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/f376161e.jpg)

## Linux中PATH、 LIBRARY\_PATH、 LD\_LIBRARY\_PATH的区别

> 参考[Linux中PATH、 LIBRARY\_PATH、 LD\_LIBRARY\_PATH的区别](https://link.zhihu.com/?target=https%3A//www.imooc.com/article/43747)
...
到 [CUDA Toolkit Download](https://link.zhihu.com/?target=https%3A//developer.nvidia.com/cuda-downloads) 下载所需版本，以 cuda\_9.0.176\_384.81\_linux.run为例：

![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/e1d27fe4.jpg)
...
## Conda安装的CUDA Toolkit和NVIDIA的CUDA Toolkit到底什么区别？

上面说了那么多可能看得云里雾里，下面做一个小的总结（参考下文）。

[Pytorch 使用不同版本的 cuda](https://link.zhihu.com/?target=https%3A//www.cnblogs.com/yhjoker/p/10972795.html)
...
啰嗦了这么多希望把CUDA相关的概念说清楚了，有用就

* 点个赞
* 打个赏
* 关注下公众号

![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/82409f27.jpg)
...
![](/assets/img/marsggbo/2019-11-11-显卡显卡驱动nvcc-cuda-drivercudatoolkitcudnn到底是什么/228f26c5.jpg)
