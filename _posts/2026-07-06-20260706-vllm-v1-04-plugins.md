...
vllm v1 给出了另一种答案：**插件系统**。背后的机制有两层：

- **Python 标准库的 `entry_points` 机制**：你写一个独立包，在 `pyproject.toml` 里声明几行，`pip install` 之后 vllm 启动时自动发现加载——这是 Python packaging 生态的标准插件总线
- **字符串注册模式**：vllm 里所有可替换的类（Worker、Platform、attention backend）都以字符串 qualname 而非类对象注册，等到真正要用时才 `import`——避免 CUDA 在主进程被提前初始化

这两层机制加在一起，让你不改 vllm 一行代码就能扩展它。插件系统的入口在 `vllm/plugins/__init__.py`，四类扩展点分别是：
