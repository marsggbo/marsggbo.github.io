...
早几年是端到端的声学模型（CTC、RNN-T），后来 Whisper 出来，直接把语音识别的天花板抬了一截。再后来，大家发现把 Whisper 当编码器、接一个 LLM 来解码，精度还能再往上走——毕竟 LLM 有大量文本知识，能更好地处理同音词、专有名词这些边缘情况。
...
但直接把 dLLM 搬来做 ASR，又踩了一堆新坑。这篇 **dLLM-ASR** 就是来填这些坑的，最终在 LibriSpeech 等多个数据集上实现了 **4.44× 推理加速**，同时 WER 比自回归 LLM-ASR 还要低一点点。
...
精度确实好，但推理慢——8B 参数的 LLM，一个词一个词地生成，RTF（实时因子，处理时间/音频时长）在 LibriSpeech clean 上是 **0.317**，也就是处理 1 秒音频需要 0.317 秒，还谈不上实时。
...
dLLM 推理从全 mask 出发，没有任何先验。但 ASR 任务本身信息量非常丰富——音频已经告诉你这句话大概多少个字、每个位置发什么音。从零开始去噪，等于白白忽略了这些信息，需要更多步骤才能收敛。

**矛盾二：序列长度固定**

dLLM 要预设一个固定的生成长度。但 ASR 的转录结果长短差异很大，短句子会产生大量 padding token，这些 token 同样参与每一步的去噪计算，纯属浪费。

**矛盾三：去噪步骤固定，不分难易**

每个 token 收敛速度不同：有些 token 第一步就高置信度确定了，有些 token 需要跑很多步才能稳定。固定步数下，"简单"的 token 也要跑完全程，整体计算量远超必要值。
...
- **Stage 2**：Adapter + LoRA（r=16, scaling=32, dropout=0.05）联合微调
...
在冻结的 Whisper 编码器输出上加一个轻量的 CTC 分支（1D 下采样卷积 + 分类头），生成一个初始的 ASR 假设序列。推理时：
...
**评估指标**：WER（词错误率，越低越好）和 RTF（实时因子，越低越好，RTF<1 表示快于实时）。
...
思路很清晰，三个矛盾对应四个解法，逻辑链完整。几个地方我会想进一步问：
...
这篇文章结尾提到未来工作是"扩展到流式 ASR"。这个问题确实不简单：masked diffusion 的并行去噪天然是"等全句到齐了再处理"，流式场景需要来一帧处理一帧。怎么把这个 prior-guided adaptive denoising 改造成增量式，是个有意思但非平凡的挑战。

**4. RTF 在不同硬件上的可复现性**

RTF=0.063 测的是什么 GPU？batch size 多少？论文没有太详细说明实验硬件配置，这个数字在不同部署场景下差异可能很大。

当然，作为一篇解决"dLLM 用于 ASR 推理效率"这个真实痛点的工作，在 WER 不退步的前提下实现 4.44× 加速，已经是很实在的贡献。对 dLLM 推理加速感兴趣的朋友可以仔细看看它的 Speech KV Cache + Confidence-Based Denoising 组合，思路可以迁移到其他 dLLM 应用场景。
