...
dLLM 的推理是在一个预先分配好的固定长度序列上做去噪的——因为 transformer 的 attention 矩阵在计算前就得确定序列长度，没法动态扩缩。但 ASR 的转录结果长短差异很大，你不知道这段音频会说几个字。实际操作中只能预设一个够大的上限，短句子就用 padding token 补齐，这些 padding 同样参与每一步的去噪计算，纯属浪费。

**矛盾三：去噪步骤固定，不分难易**

每个 token 收敛速度不同，这很好理解：句子里有些词在上下文里几乎是唯一确定的（比如固定搭配、专有名词），第一步去噪就能高置信度预测出来；但有些词存在歧义（同音词、口音模糊的词），需要多轮去噪才能稳定。但 dLLM 的去噪步数是固定的，所有 token 不管难易一律跑满全程，"简单"token 早就确定了还在白白消耗算力。
