...
记${S(W, A), T (W^T , α^T )}$分别表示学生和教师网络（由权重W和结构A决定)。为了摆脱教师网络带来的束缚（即学生网络很可能会非常像教师网络的结构，导致bias和diversity低），教师网络和学生网络是一样的，即$\alpha^T=A$。更具体地说，教师网络是用**EMA**策略更新的Supernet，学生网络就是常规的Supernet。
