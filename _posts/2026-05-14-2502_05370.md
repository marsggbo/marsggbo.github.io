---
layout: post
title: "延迟降47%！FineMoE如何用「细粒度」打破MoE推理的显存-延迟死局"
date: 2026-05-14
tags: [LLM, MoE, 论文解读, 推理优化, Expert Offloading]
---

# 延迟降47%！FineMoE如何用「细粒度」打破MoE推理的显存-延迟死局

> 原文：[Taming Latency-Memory Trade-Off in MoE-Based LLM Serving via Fine-Grained Expert Offloading](https://arxiv.org/abs/2502.05370)
...
![MoE 推理流程与显存-延迟 trade-off 示意](/assets/img/posts/2502_05370/001_a.png)
...
![MoE serving 设计空间](/assets/img/posts/2502_05370/003_Figure_2.png)
...
![粗粒度 vs 细粒度 expert 激活热图及熵分析](/assets/img/posts/2502_05370/004_a.png)
...
![细粒度 vs 粗粒度的 expert hit rate 对比](/assets/img/posts/2502_05370/007_Figure_4.png)
...
FineMoE 的设计围绕三个目标展开：
1. 低显存占用 + 低推理延迟（打破 trade-off）
2. 最小化 expert miss（减少 on-demand 加载）
3. 自适应不同 MoE 模型和不同 prompt（不搞 one-fits-all）

### 4.1 整体架构

如下图，FineMoE 由三个核心组件构成：

![FineMoE 系统架构和工作流](/assets/img/posts/2502_05370/008_Figure_5.png)
...
五步工作流：
1. 收集推理上下文（语义 embedding + 历史 expert 激活轨迹）
2. 搜索最匹配的 expert map
3. 根据搜索结果决定 prefetch 哪些 expert
4. 推理执行（hit → 直接用；miss → on-demand 加载）
5. 更新 Expert Map Store

### 4.2 Expert Map：细粒度的核心数据结构

**Expert Map 是 FineMoE 的核心创新**。

传统方案（如 MoE-Infinity）只记录 expert 的 hit count（某个 expert 在 request 里被激活了多少次）。FineMoE 不一样——它记录每个 iteration 里，gate 网络输出的**完整概率分布**。

如下图，expert map 记录了每一层 gate 输出的概率分布：

![Expert Map 数据结构示意](/assets/img/posts/2502_05370/009_Figure_6.png)
...
![Expert Map 搜索工作流（语义搜索 + 轨迹搜索）](/assets/img/posts/2502_05370/010_Figure_7.png)
...
![语义/轨迹相似度与 expert hit rate 的 Pearson 相关系数](/assets/img/posts/2502_05370/012_Figure_9.png)
...
FineMoE 用 **Publisher-Subscriber 异步架构**解决了这个问题：
...
![Prefill 和 Decode 阶段整体性能对比](/assets/img/posts/2502_05370/013_Figure_10.png)
...
![在线服务请求延迟 CDF](/assets/img/posts/2502_05370/014_Figure_11.png)
...
![不同显存限制下的 TPOT 对比](/assets/img/posts/2502_05370/015_Figure_12.png)
...
如下图，在 80 GB HBM2e A100 上测试，FineMoE 的相对优势有所收窄（因为高端 GPU 的 expert on-demand 加载速度更快），但**仍然全面优于所有 baseline**：

![A100 上的性能对比](/assets/img/posts/2502_05370/016_Figure_13.png)
...
![消融实验：不同 expert 追踪方式的 hit rate 对比](/assets/img/posts/2502_05370/017_a.png)
...
1. 推理延迟高，predictor 本身的延迟有时和 MoE 推理延迟本身差不多
2. 需要 per-layer 训练，每次换模型或 workload 都要重新训
3. 参数量大，占用额外 GPU 显存
4. 和细粒度设计不兼容：在细粒度数据上训练 NN 收敛困难
...
1. **发现了一个被忽视的问题**：现有 MoE offloading 方案都在用粗粒度（request 级别）的 expert 激活模式，而 iteration 级别的细粒度模式预测准确率更高
2. **设计了一个轻量的数据结构**（expert map）来追踪细粒度模式，不需要额外训练
3. **双路搜索**（语义 + 轨迹）覆盖了 prefill 初期没有历史轨迹的盲区
4. **异步架构**让搜索和 prefetch 不阻塞推理
5. **动态阈值**让 prefetch 数量自适应搜索置信度
...
*作者：Hanfei Yu, Xingqi Cui, Hong Zhang, Hao Wang, Hao Wang（Stevens Institute of Technology / Rice University / University of Waterloo / Rutgers University）*
