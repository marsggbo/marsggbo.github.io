---
layout: post
title: "长音频转写慢 10 倍、99% 的 Attention 在白做功——MURMUR 是怎么解决的"
date: 2026-07-13
tags: [ASR, LLM, KV Cache, 推理优化, 论文解读]
---
...
现代 ASR 本质上是 Transformer 的 Encoder-Decoder（或 Decoder-only）结构。解码（生成文字）是**逐 token 自回归**的：生成第 1 个词，把这个词加入上下文，再预测第 2 个词……每一步都要 attend 到之前所有的 token。
...
![WhisperX vs VibeVoice-ASR 精度与延迟](/assets/img/posts/20260713-murmur-asr/table1_p2.png)
...
![三种 ASR 推理方式的架构对比](/assets/img/posts/20260713-murmur-asr/figure1_full.png)
...
![不同 chunk size 下的各项 error rate](/assets/img/posts/20260713-murmur-asr/figure2_p3.png)
...
![延迟与精度 trade-off 曲线](/assets/img/posts/20260713-murmur-asr/figure3_p3.png)
...
![30s vs 300s chunk 的 attention heatmap](/assets/img/posts/20260713-murmur-asr/figure4_p4.png)

左边是 30 秒 chunk 的 attention，比较密集；右边是 300 秒 chunk 的，出现了明显的**对角线稀疏模式**——解码推进到哪里，有效 attention 就集中在对应音频位置附近，前面已经转写完的音频基本用不到了。

更精确的量化：

![每层 KV cache 中各类 token 占比](/assets/img/posts/20260713-murmur-asr/figure5_p4.png)
...
Speech token 不能用同一套策略，因为它的有效 attention 区域是随着解码进度**向后平移**的（那个对角线），不是固定在最近端。

MURMUR 设计了一个**带延迟的滑动窗口**：
...
![KV cache eviction 策略消融实验](/assets/img/posts/20260713-murmur-asr/table2_full.png)
...
![各数据集 WER 随 chunk size 变化](/assets/img/posts/20260713-murmur-asr/figure7_p7.png)
