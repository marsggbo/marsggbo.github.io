---
layout: post
title: "ML tensor 搬到 GPU 的带宽不够，为什么答案不是量化？IBP 用无损 bit 压缩缓解 PCIe 瓶颈"
date: 2026-05-14
tags: [GPU, Compression, LLM Inference, GNN, DLRM, 论文解读]
---

# ML tensor 搬到 GPU 的带宽不够，为什么答案不是量化？IBP 用无损 bit 压缩缓解 PCIe 瓶颈
...
论文前面有一个很有意思的表：很多算法压缩率不错，但 transfer + decompression 后速度不一定快。比如 zStd 在 sparse GNN 上 space saving 很高，但 transfer speedup 并不好。IBP 的目标不是追求最高压缩率，而是追求**压缩后搬得更快，解得更快**。
...
如下图是 IBP 集成到 ML 应用中的两种方式：一种压缩 GPU cache，一种压缩 CPU memory 中等待搬运的数据。

![IBP 集成方式](/assets/img/posts/ibp/emb_002_p4.png)
...
这听起来不夸张，但关键是 transfer + decompression throughput。IBP 在各类数据上都有比较好的 transfer speedup：Table 1 中 IBP 在 Sparse GNN 上 9.7×，Dense GNN 1.2×，DLRM 1.1×，LLM KVCache 1.0 到 1.1×。很多通用算法虽然压得更小，但解压开销会抵消收益。
...
如下图，IBP 对不同 GNN dataset 的训练 epoch speedup 有明显提升，尤其是可压缩性高的数据。

![GNN 加速结果](/assets/img/posts/ibp/emb_003_p13.png)

DLRM 上，IBP 的收益主要来自 GPU-based lookup/transfer。baseline 需要 CPU 做 table lookup，然后把结果搬到 GPU；IBP 用 GPU zero-copy 直接并行读 CPU memory。随着 batch size 和 embedding entry size 增大，IBP 的优势更明显。论文摘要里给出的平均提升是 DLRM embedding lookup 快 180%。

LLM 上，作者把 IBP 接入 InfiniGen 的 KV-cache fetching 路径。实验使用 OPT-2.7B、6.7B、13B、30B，1920 input tokens，128 output tokens，batch size 20。IBP 让 KV-cache movement latency 最高降低 37%，整体 inference latency 最高降低 27%，平均约 25%。
...
IBP 的 decompression throughput 在 space saving 小于 50% 时能达到 ideal 的 95% 以上。也就是说，对于 dense ML tensors 这种压缩率不算极高的场景，decompression overhead 基本被 GPU parallelism 和 PCIe transfer overlap 掩盖掉了。
